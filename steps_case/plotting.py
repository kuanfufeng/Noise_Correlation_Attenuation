...
def plot_filtered_waveforms(freq, tt, wav, fname, ccomp):
    nfreq = len(freq) - 1
    fig, ax = plt.subplots(nfreq, figsize=(8, 20), sharex=False)
    ...
    fig.tight_layout()
    plt.savefig("Waveform_filtered_%s_%s_F%s-%s.png" %
                (fname, ccomp, fmin, fmax), format="png", dpi=100)
    plt.close(fig)
...
def plot_envelope(comp_arr, freq, msv, msv_mean, fname, vdist):
    ...
    fig, ax = plt.subplots(ncmp+1, nfreq, figsize=(16, 20), sharex=False)
    ...
    for fb in range(nfreq):
        ...
        ax[-1, fb].set_title("@%4.1f-%4.1f Hz" %
                             (fmin, fmax))
        ax[-1, fb].set_xlabel("Time [s]")
        ax[-1, fb].set_ylabel("Amplitude")
    plt.tight_layout()
    plt.savefig("Waveform_envelope_%s_F%s-%s.png" % (fname, fmin, fmax),
                format="png", dpi=100)
    plt.close(fig)
...
def plot_fmsv_waveforms(freq, wav, fname, noise_level, twin):
    nfreq = len(freq) - 1
    fig, ax = plt.subplots(nfreq, 2, figsize=(12, 20), sharex=False)

    for fb in range(nfreq):
        fmin = freq[fb]
        fmax = freq[fb+1]
        absy = 1  #max(wav[fb], key=abs)
        ax[fb][0].set_ylim(-0.1, absy)
        ax[fb][0].plot(wav[0], wav[fb+1], "k-", linewidth=0.5)
        ax[fb][0].set_xlabel("Time [s]")
        ax[fb][0].set_ylabel("Amplitude")
        ax[fb][0].set_title("%s  @%4.1f-%4.1f Hz" % (fname, fmin, fmax))

        ax[fb][1].plot([wav[0][0], wav[0][-1]], [noise_level[fb], noise_level[fb]],
                       c='orange', marker='o', ls='--', linewidth=2)
        ax[fb][1].plot([twin[fb][0], twin[fb][0]], [-0.1, absy],
                       c='orange', marker='o', ls='--', linewidth=2)
        ax[fb][1].plot([twin[fb][1], twin[fb][1]], [-0.1, absy],
                       c='orange', marker='o', ls='--', linewidth=2)

        ax[fb][1].set_yscale('log', base=10)
        ax[fb][1].plot(wav[0], wav[fb+1], "k-", linewidth=0.5)
        ax[fb][1].set_xlabel("Time [s]")
        ax[fb][1].set_ylabel("Amplitude in log-scale")
        ax[fb][1].set_title("%s  @%4.1f-%4.1f Hz" % (fname, fmin, fmax))
    fig.tight_layout()
...
def plot_fitting_curves(mean_free, intrinsic_b, tt, Eobs, Esyn, fname,
    ...
    for nb in range(numb):
        ...
        plt.plot([twin[0], twin[0], twin[-1], twin[-1], twin[0]],
                 [pymin, pymax, pymax, pymin, pymin], "r", linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig("Fitting_fmsv_%s_F%s-%s_MFP%.2f.png" %
                (fname, fmin, fmax, mean_free), format="png", dpi=100)
    plt.close()


# -----
def plot_fitting_result(mean_free, intrinsic_b, Q, tt, Eobs, Esyn, fname, ev,
                        dist, twin, fmin, fmax):
    ...
    plt.title("%s %s %.1fkm  @%.1f-%.1f Hz, mfp: %.1f  b: %.2f  Q: %.1f" %
              (fname, ev, dist, fmin, fmax, mean_free, intrinsic_b, Q))
    plt.xlabel("Time [s]")
    plt.ylabel("Energy density Amp")
    plt.tight_layout()
    plt.savefig("Final_fmsv_%s_%s_F%s-%s.png" % (fname, ev, fmin, fmax),
                format="png", dpi=100)
    plt.close()
...
def plot_grid_searching(sta_pair, freq, SSR, x, y):
    ...
    for fb in range(nfreq):
        ...
        grid = SSR[fb].T
        im = ax[fb].imshow(grid, extent=(x.min(), x.max(), y.max(), y.min()),
                           aspect='auto', cmap='viridis_r', interpolation='spline16')
    ...
    plt.tight_layout()
    plt.savefig("Final_SSR_%s.png" % (sta_pair), format="png", dpi=100)
