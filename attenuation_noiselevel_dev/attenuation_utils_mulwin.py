...
### -----
def get_energy_density(fmsv_single: np.ndarray, dt: float, cvel: float, r: float, npts: int, \
    mean_free_path: float, intrinsic_b:float) -> Tuple[np.ndarray, np.ndarray]:
    """
...
# ---
def scaling_Esyn(Esyn_temp: np.ndarray, Eobs_temp: np.ndarray, \
    twindow: np.ndarray, dt: float) -> np.ndarray:
    """
...
# ### -----
def windowing_SSR(coda_window: np.array, mean_free_path: float, intrinsic_b: float,\
     fmsv: np.ndarray, dist: np.ndarray, npts: int, dt: float, cvel: float, ncoda: int) -> float:
    """
    ...
    for aa in range(npair):
        ...
        Esyn_temp, Eobs_temp = get_energy_density(fmsv[aa], dt, cvel, r, npts, \
            mean_free_path, intrinsic_b)
        
        # Scale the Esyn
        scaled_Esyn, scaling_amplitude = scaling_Esyn(Esyn_temp, Eobs_temp, twindow, dt)
        #logger.info(f"pair {aa}, (mfp, intb)=({mean_free_path:.1f}, {intrinsic_b:.2f}) -- scaling amp: {scaling_amplitude:.2f}")
        
        if PLOT_CHECK:
            ax[aa].plot(Eobs_temp, color='black', ls='-', label=f'Eobs')
            ax[aa].plot(scaled_Esyn, color='blue', ls='-', label=f'Esyn')
            ax[aa].plot([twindow[0]//dt, twindow[0]//dt], [0, 1], color='red', ls='--', label=f'window')
            ax[aa].plot([twindow[-1]//dt, twindow[-1]//dt], [0, 1], color='red', ls='--')
            ax[aa].set_title(f"pair {aa} on (mfp, intb)=({mean_free_path:.1f}, {intrinsic_b:.2f}) -- scaling amp: {scaling_amplitude:.2f}")
    ...
    for nfree in range(len(mfpx)):
        ...
        for nb in range(len(intby)):
            ...
            for ncoda in range(nwindows):     
                ...
                single_window_SSR = windowing_SSR(twindow, mean_free, intrinsic_b,\
                    fmsv_mean, vdist, npts, dt, cvel, ncoda)
                
                SSR_temp = SSR_temp + single_window_SSR
                #logger.info(f"window {ncoda}, (mfp, intb)=({mean_free:.1f}, {intrinsic_b:.2f}) -- single SSR: {single_window_SSR:.2f}, final SSR: {SSR_temp:.2f}")
...
def get_optimal_Esyn(para) -> Tuple[float, float, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for ncoda in range(nwindows):              
        for aa in range(npair):
            ...
            # Get the energy density
            Esyn[aa, ncoda], Eobs[aa, ncoda] = get_energy_density(fmsv[aa], dt, cvel, vdist, npts, \
                result_mfp, result_intb)

            # Scale the Esyn
            scaled_Esyn, scaling_temp = scaling_Esyn(Esyn[aa,ncoda], Eobs[aa,ncoda], twindow, dt)
            logger.info(f"nwindow {ncoda}, pair {aa}, (mfp, intb)=({result_mfp:.1f}, {result_intb:.2f}) -- scaling amp: {scaling_temp:.2f}")
