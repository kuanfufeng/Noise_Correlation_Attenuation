...
path=sys.argv[1] 
sta_pair=sys.argv[2]
boots="ave30-30" #sys.argv[3]

noise_qt=np.ndarray(3)
noise_qt[0]=sys.argv[4]
noise_qt[1]=sys.argv[5]
noise_qt[2]=sys.argv[6]
...
ftxt="NOISE_WINDOW_"+sta_pair+"_F"+ffstr+".txt"
f0 = open(ftxt, "a")
...
line="ENV,Fband_(Hz),Noise_qt5,Amp(et),window_bt,window_et\n"
f0.write(line)
f1.write(line)
f2.write(line)
...
for nev in range(len(evd)):
    ev=evd[nev]
    inpf=sta_pair+"_"+sta_pair+"_"+ev+"_"+boots+".h5"
    ...
    for aa in range(fnum):
        ...
        for fb in range(nfreq):
            ...
            twinbe[aa][fb][0]=(1/fmin)*2
            for pt in range(len(sym)):
                if (sym[pt] < noise_qt[fb]):
            ...
            line=str(ev)+",%.1f-%.1f"%(fmin,fmax)+",%.6f"%noise_qt[fb]+",%.6f"%ampend+",%.1f,%.1f "%(twinbe[aa][fb][0],twinbe[aa][fb][1])+"\n"
